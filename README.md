# TyreDyn
TyreDyn is an open source tyre modelling package for Python. The aim is to provide a standard python interface for the 
various tyre models in existence, similar to what [MFeval](https://mfeval.wordpress.com/) is in MATLAB. The code is 
open-source and licenced under an MIT licence, and can thus be used freely in both personal and professional 
applications.

This is a personal project, and is not associated with any company or organisation I am or have been a part of.

If you find any mistakes, would like to contribute, please contact me on:
...
1. `use_model_type` -- Overrides the `FITTYP` parameter in the TIR file. Compatibility is limited to MF 5.0 ~ MF 6.2. 
   Valid inputs should have one of the following formats: `52`, `MF52`, `MF5.2`, or `MF 5.2` (default is `None`).
...
9. `use_mfeval_mode` -- Compatibility mode with MFeval. If set to `True`, the output list of `find_full_state` matches 
   that of MFeval, if set to `False`, the outputs are organised on parameter type (default is `False`).
