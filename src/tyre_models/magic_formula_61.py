...
class MF61(TyreBase):
    ...
    def find_deflection(
        ...
        return self.radius.find_deflection(FX, FY, FZ, N, P=P)
    ...
    def find_effective_radius(
        ...
        return self.radius.find_effective_radius(FZ, N, P=P)

    @wraps(RadiusMF61.find_free_radius)
    def find_free_radius(
            self,
            N: SignalLike
    ) -> SignalLike:
    ...
    def find_loaded_radius(
        ...
        return self.radius.find_loaded_radius(FX, FY, FZ, N, P=P)
    """
    #------------------------------------------------------------------------------------------------------------------#
...
if __name__ == "__main__":
    ...
    [FX, FY, FZ,  # DONE
     MX, MY, MZ,  # DONE
     SL, SA, IA, PHI, VX, P, N,  # DONE
     R_omega, RE, rho, RL,  # DONE
     a, b, t,  # DONE
     mu_x, mu_y,  # DONE
     MZR,  # DONE
     Cx, Cy, Cz,  # DONE
     KYA, iKYA, KXK, iKXK,
     sigma_x, sigma_y] = tyre.find_full_output(SA, SL, FZ, VX, N, P=P, IA=IA, VC=None, VCX=None, VS=0.0, PHI=0.0, angle_unit="deg")
