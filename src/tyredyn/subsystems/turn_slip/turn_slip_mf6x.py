...
class TurnSlipMF6x:
    ...
    def _find_zeta_4(
        ...
        KYAO_sign = self.signals._replace_value(np.sign(KYAO), target_sig=KYAO, target_val=0.0, new_val=1.0)
        ...
        # peak factor (4.86)
        VCX_sign = self.signals._replace_value(np.sign(VCX), target_sig=VCX, target_val=0.0, new_val=1.0)
