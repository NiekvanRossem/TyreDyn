...
class ForcesMF6x(SubSystemBase):
    ...
    def _find_fx_pure(
        ...
        VS, VC = self.extra_signals._find_speeds(SA=0.0, SL=SL, VX=VX)
        ...
        FX0 = self.signals._flip_negative(FX0, helper_sig=VX) if self._use_alpha_star else FX0
    ...
    def _find_fy_pure(
        ...
        # flip sign for negative speeds (only if alpha_star is used)
        FY0 = self.signals._flip_negative(FY0, helper_sig=VX) if self._use_alpha_star else FY0
